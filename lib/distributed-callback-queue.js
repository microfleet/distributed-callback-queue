...
class DistributedCallbackQueue {
  ...
  push(suffix, next) {
    ...
    // get the lock
    return lock.acquire(lockRedisKey).then(() => this.createWorker(lockRedisKey, lock)).catch(notLockAcquisitionError, err => {
      // this is an abnormal error, need to post it and cancel requests
      // so that they dont hang
      return this.publish(lockRedisKey, err).throw(err);
    });
  }
  ...
  createWorker(lockRedisKey, lock) {
    ...
    return function (err) {
      ...
      return lock.release().then(() => {
        ...
        // by someone else
        return dlock.publish.apply(dlock, [lockRedisKey, err].concat(args));
      }).catch(error => {
