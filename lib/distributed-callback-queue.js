...
function DistributedCallbackQueue(options) {
    ...
    _.extend(this, {
        ...
        publish: callbackQueue.createPublisher(pubsub, pubsubChannel),
        consume: callbackQueue.createConsumer(pubsub, pubsubChannel)
    });
...
DistributedCallbackQueue.prototype.push = function (key, next, done) {
    ...
    lock.acquire(lockRedisKey, function lockAcquireCallback(err) {
        ...
        if (err) {
            ...
            // report that we couldn't acquire lock, so that requester
            // doesn't proceed with further updates
        ...
        setImmediate(done, null, self.createWorker(lockRedisKey, lock, self.logger));
